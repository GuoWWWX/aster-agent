...
- 用户可标记优先级、截止时间和目标项目。
- 用户可编辑任意未删除 WorkItem 的标题和需求并保留审计事件。运行中的修改必须把完整新版本作为 `steer` 投递给 Team Lead；若执行根暂时空闲，则创建受同一 WorkItem 生命周期约束的协调 Run。修改不回滚已经发生的命令、文件或网络副作用。看板删除采用软删除：先从默认列表隐藏并保留 Conversation、Run、事件和协作记录，运行中还要取消执行树内的活动 Run。
- 用户可在任意 WorkItem 上追加任务级评论；评论写入 WorkItem 的持久事件流，不复制到成员 Conversation，也不唤醒 Agent。只有用户明确选择“评论并返工”或验收区返工动作时，才在原执行 Conversation 创建新 Run。已完成 WorkItem 允许显式返工，但必须增加修订号、清除当前完成态投影并保留旧验收、评论、Run 和事件历史。
...
- **任务详情**：在团队面板中显示分解依据、分配理由、状态、Artifact、Diff 和用户验收。进入待验收时，以 Team Lead 最终回复作为“完成说明”，汇总完成内容、实现功能、验证结果和未决风险；原始验收要求只读展示供核对，不要求用户逐项勾选，完整 Agent 消息不在任务详情中重复展示。

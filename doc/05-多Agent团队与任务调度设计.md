...
### 3.7 主对话与 Subagent 协作优化

> 本节为当前协作规则，优先于旧的等待与唤醒描述；复用现有消息、JSONL 和执行机制，不另建一套协作系统。

| 场景 | 处理方式 |
| --- | --- |
| 创建或继续 Subagent | 立即返回，后台执行；后续任务复用原有子对话与上下文，不重建对话；空闲时开启新执行轮次，运行中在安全节点接收消息 |
| Subagent 返回结果 | 先追加到主对话 JSONL，形成持久结果队列，不直接发起模型请求 |
| 主对话正在工作 | 在下一次模型请求前批量接收待处理结果，不打断当前生成或工具执行 |
| 主对话空闲 | 在固定 200 毫秒窗口内合并结果再唤醒；后续到达不延长窗口，每个主对话最多一个待执行唤醒，不创建并行 Run |
| 主对话选择等待 | 挂起到指定对象满足 `any/all` 条件；结果仍入队，条件未满足不因单条回执重复唤醒 |
| 用户停止主对话 | 停止所属运行中 Subagent，递归覆盖已有嵌套子代理；取消等待、待处理审批及待执行唤醒 |
| 主对话正常输出结束 | 不停止 Subagent，后续结果仍可唤醒主对话 |

**等待与停止**

- `timeoutMs` 可选，省略不设置业务等待超时；固定等待集合为指定子对话的当前 `runId`，新任务不自动加入。普通协作默认合并唤醒，需要整批结果时选择 `all`；短暂合并本身不保证整批完成。
- 完成、失败、取消、删除均解除对应等待，`all` 不要求全部成功。等待超时不取消 Subagent，也不代表执行失败。
- 等待不阻塞界面、不持续请求模型；用户可取消或发新要求。每秒检查执行是否仍存在；非审批状态连续 5 分钟无进展时返回 `interrupted/no_progress`，不停止子任务。审批等待不计入这个期限。重启后明确中断未结束执行，恢复终态与回执，不静默重放命令。
- 主对话的主动停止不波及仅通信过的独立对话或整个团队；保留历史、结果和已有文件改动，不自动回滚。停止与结果返回同时发生时，停止优先禁止自动唤醒；迟到回执和重启恢复也不能让已停止的任务自行续跑。用户继续后再决定是否复用原 Subagent。

**结果回执与按需读取**

- 回执包含：唯一消息 ID、来源名称、`conversationId`、`runId`、执行状态、最终说明、错误及系统实际记录的文件变更；不把未知的 Shell 间接修改冒充完整变更清单。
- 没有总结也必须送达结束通知，标记“未提供总结”；不把过程描述冒充最终总结，不把执行结束当作验收通过。
- 主对话自行决定是否用 `read_agent_conversation` 读取该轮问答、工具调用和结果，或通过 `query` 搜索；仍不足时让原 Subagent 补充说明或返工。不自动复制完整子对话，也不新增专用读取工具。
- 回执按 ID 去重；批次绑定到接收 Run 并持久确认消费后才从待处理队列移除，历史不删除。崩溃恢复不得漏结果、重复注入或重复创建同一次唤醒。
- 生命周期保持“工作中 → 本轮完成 → 主对话验收后已结束”；失败、取消记录为本轮执行结果，仍可继续原对话。删除只由用户操作。

**工具与持久化**

`read_agent_conversation` 使用 JSONL，支持 `conversationId + runId` 限定本轮，以及 `query`、Token 预算和游标分页；指定 `runId` 时不受其他轮次压缩边界影响。`wait_for_subagents` 继续使用创建时返回的 `taskIds`，调用时固定各子对话当前轮次；`timeoutMs` 可省略，`waitFor` 为 `any/all`。

停止状态写入 `conversation_execution_paused`；消费批次写入 `agent_messages_consumed`，包含接收 `runId` 和消息 ID。回执保留 `executionStatus`、`summaryStatus`、`executionError`；消费消息保留 `consumedByRunId`，用于恢复执行结束但尚未回传的结果。状态检查点只保留有界结束摘要，不复制完整子对话。

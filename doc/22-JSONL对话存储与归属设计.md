...
> 2026-09-07。§1 为迁移现状〔FACT〕；其余为最终格式〔INFER〕。第 23 篇及 `doc/examples/` 展示最终 v2 场景，最新属性规则以本文为准。

## 1. 当前存储

- 日志：`<ASTER_HOME>/conversations/<conversationId>/conversation.jsonl`；旧平铺文件启动时迁入该目录，冲突时保留旧文件备份。
- 新对话、模型上下文和常用 Run 事件已由 JSONL 驱动；标题、项目/工作区、Agent、模型/思考、权限、归档和置顶会追加属性快照并可恢复 SQLite 投影。
- 列表、搜索、团队关系及少数跨对话原子操作仍读 SQLite 投影；完全移除对话表须先完成 v2 读取器、内存索引和跨日志提交协议。
- 项目归属来自属性，不从文件路径推算。
...
| 列表、搜索、状态查询 | 从 JSONL 重建内存索引；以后可选持久化缓存，但必须可重建 |
...
- 启动扫描日志建立内存索引，打开对话读取 JSONL，后续增量更新。首次属性读取需重放，之后读内存；修改只追加。耗时取决于日志量、磁盘和刷盘策略。
- 目标备份包含该对话 JSONL、附件、产物及其已有的 `workspaces/<conversationId>/`；父/子对话、团队数据、项目代码和手动绑定目录另行纳入，见[第 21 篇 §10](./21-数据库实体关系图.md)。实际迁移完成前，旧版对话仍须同时备份 SQLite。
- 导入遇到同 ID 冲突、损坏或未知版本时保护原件；引用缺失可看历史，执行前重新绑定配置与工作区。
- 路径禁止越界；正文和配置按私有数据保护。恢复不重跑已完成工具、不恢复旧审批授权。
- 迁移须核对旧 SQL、JSONL 的全部属性、历史和附件；验证新版读写后才停止旧对话表读写，保留旧备份。

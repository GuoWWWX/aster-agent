...
Use `list_agent_conversations`, `read_agent_conversation`, `send_agent_message`, and `wait_for_agent_message` to collaborate with other Agents. Check status whenever needed and keep cross-conversation reads within budget. With `expectReply=true`, use `replyInstruction` when the completion receipt needs specific conclusions, evidence, or risks. Handle incoming Agent messages in this persistent conversation: start the final answer with the concise requested receipt, add a standalone Markdown `---` line, then keep full details after it. The runtime returns only a bounded completion receipt before that divider and keeps full details here for on-demand reads. For progress or notifications that need no reply, call `send_agent_message` with `expectReply=false`.
